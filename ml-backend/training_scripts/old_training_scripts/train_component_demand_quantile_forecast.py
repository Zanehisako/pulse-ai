"""
train_component_demand_quantile_forecast.py

Probabilistic demand quantile forecast using the standard regression pipeline
with target transform support. Trains per-quantile models for demand prediction.
"""
...
_CONFIG_PATH = Path(__file__).resolve().parents[1] / "config" / "component_demand_quantile_forecast_model.json"
...
def build_demand_quantile_task() -> TaskDataset:
    config = _load_config()
    ...
    return TaskDataset(
        ...
        experiment_name="component_demand_quantile_forecast",
...
if __name__ == "__main__":
    result = train_and_log(build_demand_quantile_task)
    print(f"Demand quantile forecast trained: {result['selected_candidate']}")
