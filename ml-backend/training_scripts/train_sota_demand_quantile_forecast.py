# Cell 6 — Global Model 1: Demand quantile forecast
# Usage: python train_sota_demand_quantile_forecast.py
...
from sota_training_utils import (
    ...
    encode_ohe, fit_tabular_regressor, regression_metrics,
    fit_sota_regression_baselines, choose_winner,
    evaluate_regression_gate, save_bundle, save_json,
...
def main():
    print("=" * 70)
    print("Global Model 1: Demand Quantile Forecast")
    print("=" * 70)
    ...
    if inv_f_path.exists() and inv_l_path.exists():
        import pandas as pd
        inv_features = pd.read_parquet(inv_f_path)
        inv_labels = pd.read_parquet(inv_l_path)
        print(f"Loaded inventory features: {inv_features.shape}")
        print(f"Loaded inventory labels: {inv_labels.shape}")
    else:
        print(f"Data not found at {DATA_DIR}. Run generate_sota_dataset.py first.")
        return

    inventory_df = inv_features.merge(
        inv_labels[["hospital_id", "blood_type", "component_type", "event_timestamp", "units_used_next_7d"]],
    ...
    leakage_audit(forecast_features, ["*next*", "*future*", "*label*", "*target*"], "global_sota_demand")

    train, valid, test = unseen_hospital_split(inventory_df)
    target = "units_used_next_7d"
    ...
    sota = fit_sota_regression_baselines(train, valid, test, forecast_features, target, "demand_quantile_forecast")
    ...
    metrics = {"task": "demand_quantile_forecast", "primary_model": "neural_ft_transformer", "winner": winner,
    ...
    readiness_gates["global_sota_demand_quantile_forecast_model"] = evaluate_regression_gate(
    ...
    save_bundle("global_sota_demand_quantile_forecast_model", bundle, metrics)
    all_model_metrics["global_sota_demand_quantile_forecast_model"] = metrics

    print(f"\nDemand forecast — Winner: {winner}")
    print(f"  Neural: WAPE={neural_metrics['wape']:.4f} RMSE={neural_metrics['rmse']:.2f} R²={neural_metrics['r2']:.4f}")
    print(f"  Tabular: WAPE={tab_metrics['wape']:.4f} RMSE={tab_metrics['rmse']:.2f} R²={tab_metrics['r2']:.4f}")
