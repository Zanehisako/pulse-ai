# Cell 10 — Donor Model B: Contact response propensity (tabular only, proxy)
# Usage: python train_sota_donor_contact_response.py
...
from sota_training_utils import (
    ...
    evaluate_classification_gate, save_bundle, save_json,
...
def main():
    print("=" * 70)
    print("Donor Model B: Contact Response Propensity (tabular)")
    print("=" * 70)
    ...
    donor_features_cols = apply_donor_proxy_policy(
        [c for c in donor_features_cols if c in donor_df.columns], "donor_contact_response"
    )
    ...
    CONTACT_TARGET_CANDIDATES = {
        "donor_contact_response_model": ["responded_to_contact", "contact_response", "contact_response_label"],
    }
    target = next((c for c in CONTACT_TARGET_CANDIDATES["donor_contact_response_model"] if c in train.columns), None)

    if target is None or len(np.unique(train[target].astype(int))) <= 1:
        print("No real contact response label found — saving proxy-only model.")
        proxy_metrics = {
            "task": "donor_contact_response_proxy", "winner": "tabular_gbdt",
            "proxy_only": True, "operational_ready": False,
            "note": "Uses donated_next_6m as contact-readiness proxy. Requires real contact-response labels.",
        }
        # Use the next-donation model's bundle as proxy
        ...
        save_bundle("global_sota_donor_contact_response_model", bundle, proxy_metrics)
        all_model_metrics["global_sota_donor_contact_response_model"] = proxy_metrics
        readiness_gates["global_sota_donor_contact_response_model"] = {
    ...
    contact_sota = fit_sota_classification_baselines(train, valid, test, donor_features_cols, target, "donor_contact_response")
    ...
    save_bundle("global_sota_donor_contact_response_model", contact_bundle, contact_metrics)
    all_model_metrics["global_sota_donor_contact_response_model"] = contact_metrics
    readiness_gates["global_sota_donor_contact_response_model"] = evaluate_classification_gate(
        contact_test_comparison[contact_winner]
    )

    print(f"\nDonor contact response — Winner: {contact_winner}")
